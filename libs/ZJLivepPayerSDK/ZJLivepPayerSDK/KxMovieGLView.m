...
NSString *const vertexShaderString = SHADER_STRING//顶点着色器
(
...
NSString *const rgbFragmentShaderString = SHADER_STRING//片段着色器
(
             ...
             decoder: (KxMovieDecoder *) decoder
{
    ...
    if (self) {
        ...
        glGenFramebuffers(1, &_framebuffer);//创建一个帧缓冲
        glGenRenderbuffers(1, &_renderbuffer);//创建一个渲染缓冲对象
        glBindFramebuffer(GL_FRAMEBUFFER, _framebuffer);//将_framebuffer绑定到当前帧缓冲
        glBindRenderbuffer(GL_RENDERBUFFER, _renderbuffer);//把渲染缓冲对象绑定，这样所有后续渲染缓冲操作都会影响到当前的渲染缓冲对象：
        [_context renderbufferStorage:GL_RENDERBUFFER fromDrawable:(CAEAGLLayer*)self.layer];
        glGetRenderbufferParameteriv(GL_RENDERBUFFER, GL_RENDERBUFFER_WIDTH, &_backingWidth);
        glGetRenderbufferParameteriv(GL_RENDERBUFFER, GL_RENDERBUFFER_HEIGHT, &_backingHeight);
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, _renderbuffer);//把帧缓冲对象附加上
        
        GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);//为帧缓冲创建一些附件，还需要把这些附件附加到帧缓冲上
...
- (void)dealloc
{
    ...
    if (_framebuffer) {
        glDeleteFramebuffers(1, &_framebuffer);//删除帧缓冲对象
        _framebuffer = 0;
    }
